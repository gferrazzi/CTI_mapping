...
%%RGRID...
if strcmp(typ,'tukeyC') || strcmp(typ,'tukeyIsoC');ck=(N+1)/2;else ck=ceil((N+1)/2);end
...
elseif strcmp(typ,'FractionalFiniteDiscreteIso') || strcmp(typ,'FractionalFiniteDiscreteIsoNorm') || strcmp(typ,'FractionalIso')
    ...
    x=x.^gibbsRing;
elseif strcmp(typ,'tukeyIso') || strcmp(typ,'tukeyIsoC')
    ...
    x(kk>=pi)=0;kk=[];
elseif strcmp(typ,'tukey') || strcmp(typ,'tukeyC')
    N(end+1:2)=1;x=single(ones(N));
    if gpu>0;x=gpuArray(x);end
    gibbsRing(end+1:ND)=gibbsRing(end);
    for m=1:ND
        Naux=ones(1,ND);Naux(m)=N(m);spaux=ones(1,ND);spaux(m)=sp(m);        
        if strcmp(typ,'tukey');tukAux='tukeyIso';else tukAux='tukeyIsoC';end
